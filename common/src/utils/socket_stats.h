...
inline void on_open(int fd, const char* name) {
...
inline void log(int pending_threads, int sessions) {
    std::lock_guard lock(s_mutex);
    ...
    char names[192] = {};
    int pos = 0;
    for (int i = 0; i < nc_len && pos < (int)sizeof(names) - 1; ++i) {
        pos += snprintf(names + pos, sizeof(names) - pos,
                        "%s%s:%d", i ? " " : "", nc[i].name, nc[i].n);
    }

    Logger::info("Sockets: %d open (peak %d, %u total) [%s] | threads: %d | sessions: %d",
